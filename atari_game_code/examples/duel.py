...
def createLayers():
    ...
    if args.advantage == 'avg':
        ...
        z = Lambda(lambda a: K.expand_dims(a[:, 0] ) + a[:, 1:] - K.max(a[:, 1:], keepdims=True),
                   output_shape=(env.action_space.n,))(y)
    elif args.advantage == 'naive':
        z = Lambda(lambda a: K.expand_dims(a[:, 0] ) + a[:, 1:], output_shape=(env.action_space.n,))(y)
    else:
        assert False
...
plot_model(model, to_file='../result/duel.png',show_shapes=True)
